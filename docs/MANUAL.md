...
**yoyo 不是通用编程语言**。它主要面向"字节流 → 汇编发射"这类系统级代码——编译器、
加载器、文件解析器、自测试工具——而不是应用层业务逻辑。

> **关于"能 / 不能"的范围**：yoyo 当前支持字符串字面量、文件 I/O、SSE2 双精度浮点
> 运算；但**没有**网络、GUI、GPU 调度，也不能跑大型 LLM。
> 这些边界在 §9.1 列得清楚，请先看那一节再判断 yoyo 适不适合你的场景。
...
| 特性                | 状态                                  |
|---------------------|---------------------------------------|
| 字符串字面量        | ✅ 支持（opcode `12`，data section）   |
| 文件 I/O            | ✅ 支持（opcode `50`/`51`，IAT 已接）  |
| 浮点（SSE2 双精度） | ✅ 支持（opcode `90` 等，XMM 寄存器）  |
| 位运算              | Phase 2                               |
...
- **Phase 2**：补齐剩余 ~25 个 opcode，重点是位操作、条件跳转、字符串运行时操作
  （字符串字面量 / 文件 I/O / 浮点已可用，详见 §9.1）
