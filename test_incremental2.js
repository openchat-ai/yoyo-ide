...
function buildTest(prefix, codeGenFn) {
  ...
  codeGenFn(C, lr, ii, fx, jc, lm, ar, E);
  ...
  pe.setData(data);
  var fname='test2_'+prefix+'.exe';
  FS.writeFileSync(fname, pe.build());
  console.log(fname+': code='+C.tell()+'B');
}

// Open, GetFileSize, read 4096 bytes into R12, close
buildTest('readfile', function(C, lr, ii, fx, jc, lm, ar, E) {
  E.sub_ri(C,E.RSP,0x60);
  lr(E.R12,0); // data base
  lr(E.R14,0x50000);
  lr(E.RCX,0x50200+1006); // "input.ky"
  ...
  E.mov_rr(C,E.R15,E.RAX); // hFile

  // GetFileSize
  E.mov_rr(C,E.RCX,E.R15);
  E.xor_rr(C,E.RDX,E.RDX);
  ii('KERNEL32.dll','GetFileSize');
  E.mov_rr(C,E.RBP,E.RAX); // file_size

  // ReadFile(hFile, R12, 4096, &bytes_written, NULL)
  E.mov_rr(C,E.RCX,E.R15);
  E.mov_rr(C,E.RDX,E.R12);
  E.mov_ri(C,E.R8,4096);
  lr(E.R9,0x50200+1006+12); // bytes_written
  E.mov_mi32(C,E.RSP,0x20,0);
