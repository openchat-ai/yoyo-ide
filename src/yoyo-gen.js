...
const TEXT_VS    = 0x9000;   // .text virtual size (must fit emitted code)
const CODE_RVA   = 0x1000;   // .text base RVA
const IAT_BASE   = CODE_RVA + TEXT_VS; // 0xA000
...
C('Copy startup code (+ overlay) to output .text');
L(GET(0x4C, 0x03));
L('84 4C ' + hx(STARTUP_BLOB_OFF, 4) + ' 00 ; __STARTUP_TOTAL__');
...
if (useNativeOverlay) {
  ...
  // Also copy overlay to .text at compile time (required for CALL startup)
  C('Compile-time copy of handler overlay to output .text for startup');
  L(GET(0x47, 0x08));
  L(ADD(0x47, HANDLER_IMAGE_OFF));
  L(GET(0x48, 0x03) + ' ; state_48 = write_base');
  L(ADDV(0x48, 0x01) + ' ; state_48 += startupLen (text offset for overlay)');
  ...
  L(ADDV(0x0D, 0x0C));
  L(SET(0x09, 1) + ' ; suppress H_31 first-handler exit prologue during scan');
  L(SET(0x0E, 0x80) + '  ; code_offset = STARTUP_MAX (past startup+header)');
  L(CH(1)  + ' ; call H_01 main scan loop (H_CC ret lands here for post-scan driver)');
}
...
C('Write output ELF');
// Emit exit code BEFORE WriteFile so it's included in the output
if (isLinux) {
  C('Copy Linux exit syscall bytes into output .text and terminate');
  L(GET(0x4C, 0x03));
  L(ADDV(0x4C, 0x0E) + '  ; state_4C += code_offset (exit blob goes AFTER emitted code)');
...
L('51 02 01 0E');
L(RET() + '  ; return from H_00 to top-level call');
B();
...
L(H(0x4A));
L(SET(0x41, 0x71)); L(CMP(0x40, 0x41)); L(JNE(0x9E));
L(SET(0xF0, 0x84));
L(CH(0x9C) + '  ; opcode 0x71: emit JE via generic jcc');
L(JMP(0x4C));
B();

L(H(0x9E));
L(SET(0x41, 0x82)); L(CMP(0x40, 0x41)); L(JNE(0x9F));
L(SET(0xF0, 0x8C));
L(CH(0x9C) + '  ; opcode 0x82: emit JL via generic jcc');
L(JMP(0x4C));
B();

L(H(0x9F));
L(SET(0x41, 0x83)); L(CMP(0x40, 0x41)); L(JNE(0x4B));
L(SET(0xF0, 0x8F));
L(CH(0x9C) + '  ; opcode 0x83: emit JG via generic jcc');
L(JMP(0x4C));
B();
...
L(H(0xBD));
L(SET(0x41, 0xA1)); L(CMP(0x40, 0x41)); L(JNE(0xBE));
L(CH(0xA8) + '  ; opcode 0xA1: inline byte emit');
L(JMP(0x4C));
B();

C('H_BE: opcode 0xEB - emit x86 JMP rel8 (jmp-over-filename trick)');
L(H(0xBE));
L(SET(0x41, 0xEB)); L(CMP(0x40, 0x41)); L(JNE(0x4C));
L(CH(0xBF) + '  ; opcode 0xEB: emit raw x86 JMP rel8');
L(JMP(0x4C));
B();
...
L(H(0x25));
L(CMP(0x0C, 0x0D)); L(JAE(0xCC) + '  ; EOF -> H_CC');
...
C('H_1E: emit auto-ret, resolve fixups, patch startup JMP, write output');
L(H(0x1E));
L(SET(0x45, 0xC3));
...
// Patch startup call rel32 to H_00 (use handler_table[0] for correct emitted-code address)
L(SET(0x50, 0));
L(CH(0xE7) + '  ; state_4D = handler_table[0]');
const callEnd = isLinux ? 33 : 100;
L(SUB(0x4D, callEnd) + '  ; rel32 = handler[0] - call_end');
L(GET(0x4C, 0x03));
L(ADDV(0x4C, 0x01) + ' ; state_4C = write_base + startupLen');
const subOff = isLinux ? (outputStartup.length - 29) : (outputStartup.length - 96);
L(SUB(0x4C, subOff) + '  ; state_4C -= ' + subOff + ' (rel32 pos)');
L('55 4C 4D');
L(SET(0x0E, tplWriteLen) + '  ; reset code_offset for final write');
L('51 02 01 0E' + '  ; write output (after fixup resolver)');
// Exit directly — do NOT return to scanner (which would restart compilation)
C('Emit exit syscall: xor rdi,rdi; mov rax,60; syscall');
L(SET(0x45, 0x48)); L(CH(0xE0));
L(SET(0x45, 0x31)); L(CH(0xE0));
L(SET(0x45, 0xFF)); L(CH(0xE0));
...
C('H_CB: EOF finalizer (called from H_CC)');
L(H(0xCB));
L(CH(0x1E) + '  ; call H_1E');
...
// ── Helper: H_BF - emit x86 JMP rel8 (EB + arg) ──────────────────────────────
C('H_BF: emit x86 JMP rel8: EB then state_50 (first arg = JMP displacement)');
L(H(0xBF));
L(SET(0x45, 0xEB));
L(CH(0xE0));
L(GET(0x45, 0x50));
...
C('H_FC: branch opcode already emitted — return (caller records fixup + placeholder)');
L(H(0xFC));
L(RET());
B();
...
// ── H_9C/H_9D: generic jcc rel32 emitter (state_F0 = x86 cond byte) ────────
// Used by H_4A (JE 0x71/0x84), H_9E (JL 0x82/0x8C), H_9F (JG 0x83/0x8F).
